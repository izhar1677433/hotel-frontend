...
function AdminRooms() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`${BASE_URL}/api/admin/rooms/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  return (
  <div className="min-h-screen bg-gray-100 p-2 sm:p-4 md:p-6 pt-[100px] md:pt-[150px]">
  <h1 className="mb-6 text-2xl sm:text-3xl font-bold text-gray-800 text-center">Admin Panel</h1>

      {/* Tab Navigation */}
      <div className="mb-6 border-b border-gray-200">
        <nav className="-mb-px flex flex-wrap gap-2 sm:gap-0 sm:space-x-8 justify-center">
          <button
        ...
        <div>
          <div className="mb-4 flex flex-col sm:flex-row items-center justify-between gap-2">
            ...
            <button
              ...
              className="rounded bg-green-500 px-3 py-2 text-white text-sm sm:text-base transition-colors hover:bg-green-600 w-full sm:w-auto"
            >
          ...
          {/* FORM MODAL */}
          {showForm && (
            <div className="z-60 fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 px-2">
              <div className="w-full max-w-lg rounded bg-white p-2 sm:p-6 shadow-lg">
                ...
                >
                  ...
                  {imagePreview && (
                    <img
                      src={imagePreview}
                      alt="Preview"
                      className="mb-2 h-20 w-28 sm:h-28 sm:w-40 rounded-lg border-4 border-blue-200 object-cover shadow-lg transition-transform duration-300 hover:scale-105"
                    />
                  )}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {facilitiesList.map((facility, idx) => {
                        ...
                        return (
                          <label
                            key={idx}
                            className="flex cursor-pointer items-center space-x-2 rounded border p-2 text-xs sm:text-sm hover:bg-gray-100"
                          >
                            <input
                              type="checkbox"
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-2 sm:space-x-2">
                    <button
                      ...
                      className="rounded bg-gray-400 px-3 py-2 text-white text-sm sm:text-base hover:bg-gray-500"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      type="submit"
                      className="rounded bg-blue-500 px-3 py-2 text-white text-sm sm:text-base hover:bg-blue-600"
                      disabled={
                        !!fieldErrors.price ||
                        !!fieldErrors.size ||
                        !!fieldErrors.maxPerson ||
          ...
          {/* ROOMS TABLE */}
          <div className="overflow-x-auto rounded-lg bg-white shadow-md mt-4">
            <table className="w-full border-collapse text-xs sm:text-sm">
              <thead>
                <tr className="bg-gray-200">
                  <th className="border p-1 sm:p-2">Image</th>
                  <th className="border p-1 sm:p-2">Name</th>
                  <th className="border p-1 sm:p-2">Details</th>
                  <th className="border p-1 sm:p-2">Price/night</th>
                  <th className="border p-1 sm:p-2">Max Person</th>
                  <th className="border p-1 sm:p-2">Size</th>
                  <th className="border p-1 sm:p-2">Facilities</th>
                    ...
                    <td className="border p-1 sm:p-2">
                      {room.image && (
                        <img
                          ...
                          alt={room.name}
                          className="mx-auto h-16 w-24 sm:h-24 sm:w-36 rounded-lg border-4 border-blue-100 object-cover shadow-md transition-transform duration-300 hover:scale-105"
                          style={{ background: "#f3f4f6" }}
                        />
                      )}
                    </td>
                    <td className="border p-1 sm:p-2">{room.name}</td>
                    <td className="border p-1 sm:p-2">{room.description}</td>
                    <td className="border p-1 sm:p-2">${room.price}</td>
                    <td className="border p-1 sm:p-2">{room.maxPerson}</td>
                    <td className="border p-1 sm:p-2">{room.size} m²</td>
                    <td className="border p-1 sm:p-2">
